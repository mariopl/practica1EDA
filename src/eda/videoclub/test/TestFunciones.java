...
public class TestFunciones {
	...
	@Test
	public void test1(){
		Set<Pelicula> euros = videoteca.getPeliculasCambioDivisa();
		
		for (Pelicula p: euros){
			System.out.println(p.toString());
		}
		
		assertTrue("El método peliculasRecaudacionesEuros no es correcto", videoteca.size() == 5);
	}
	
	@Test
	public void test2(){
		Set<String> directores = videoteca.getDirectores();
		
		for (String p: directores){
			System.out.println(p.toString());
		}
		
		assertTrue("El método peliculasDirectores no es correcto", videoteca.size() == 5);
	}
